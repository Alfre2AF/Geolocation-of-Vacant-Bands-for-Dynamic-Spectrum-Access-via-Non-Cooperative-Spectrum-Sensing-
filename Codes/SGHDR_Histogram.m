...
runs = 1000;           % Número de rodadas de sensoriamento para computar as CDFs empíricas.
...
% Mostrar algunas métricas calculadas
...
% --- Histograma ---
figure(1);
histogram(Frac_SHG_correct, 10, 'Normalization', 'probability', 'FaceColor', 'blue'); 
axis([0 1 0 Inf]); % Limitar eje x de 0 a 1
xlabel('SHG detection rate');
ylabel('Probability'); 
title('Histograma da SHGDR');

% Calcular la media de la tasa de detección SHG
mean_detection_rate = mean(Frac_SHG_correct);

% Mostrar como texto dentro del histograma
text(0.65, 0.8, ...
     ['SHGDR mean: ' num2str(round(mean_detection_rate*100,2)) ' %'], ...
     'Units', 'normalized', ...
     'FontSize', 11, ...
     'Color', 'black', ...
